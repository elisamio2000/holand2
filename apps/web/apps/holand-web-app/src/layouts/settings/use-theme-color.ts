...
function getLocalStoragePreset() {
  if (typeof window !== 'undefined') {
    const localStorageValue = localStorage.getItem('holand-preset');
...
const colorPresetsAtomWithPersistence = atom(
  ...
  (get, set, newStorage: any) => {
    ...
    localStorage.setItem('holand-preset', JSON.stringify(newStorage));
  }
);
...
const colorPresetNameAtom = atom(
  typeof window !== 'undefined'
    ? localStorage.getItem('holand-preset-name')
    : DEFAULT_PRESET_COLOR_NAME
);
...
const colorPresetNameAtomWithPersistence = atom(
  ...
  (get, set, newStorage: any) => {
    ...
    localStorage.setItem('holand-preset-name', newStorage);
  }
);
