...
import {
  CONTEXTUAL_ASSISTANT_PANEL_STATE_EVENT,
  requestContextualAssistantOpen,
  resolveContextualAssistantSurface,
  type ContextualAssistantPanelStateDetail,
...
export default function HeaderAiChatLauncher() {
  ...
  const surface = resolveContextualAssistantSurface(pathname);
  ...
  useEffect(() => {
    ...
    const onState = (e: Event) => {
      const ce = e as CustomEvent<ContextualAssistantPanelStateDetail>;
      if (ce.detail?.surface === surface) {
        setPanelOpen(!!ce.detail.open);
      }
    };
    window.addEventListener(CONTEXTUAL_ASSISTANT_PANEL_STATE_EVENT, onState as EventListener);
    return () =>
      window.removeEventListener(CONTEXTUAL_ASSISTANT_PANEL_STATE_EVENT, onState as EventListener);
  }, [surface]);
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      requestContextualAssistantOpen(surface, rect);
    },
    [surface, panelOpen]
  );
  ...
  const label = panelOpen
    ? t('contextualAssistant.headerPanelOpen', 'Assistant panel is open')
    : t('contextualAssistant.headerOpenPanel', 'Open assistant panel');
