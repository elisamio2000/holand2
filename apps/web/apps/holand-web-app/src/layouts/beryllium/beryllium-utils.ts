...
const holandBerylliumSidebarLeftExpandedAtom = atom(
  typeof window !== 'undefined' ? localStorage.getItem(LOCAL_STORAGE_KEY) : true
);

const holandBerylliumSidebarLeftExpandedAtomWithPersistence = atom(
  (get) => get(holandBerylliumSidebarLeftExpandedAtom),
  (get, set, newStorage: any) => {
    set(holandBerylliumSidebarLeftExpandedAtom, newStorage);
...
export function useBerylliumSidebars() {
  const [expandedLeft, setExpandedLeft] = useAtom(
    holandBerylliumSidebarLeftExpandedAtomWithPersistence
  );
