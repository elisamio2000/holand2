...
interface WorkspaceCasePickerProps {
  value: string;
  onChange: (caseId: string, label?: string) => void;
  excludeIds?: string[];
}

export default function WorkspaceCasePicker({
  value,
  onChange,
  excludeIds = [],
}: WorkspaceCasePickerProps) {
  ...
  const search = useCallback(
    async (q: string) => {
      setLoading(true);
      try {
        const res = await caseImporterService.listCases({
          q: q || undefined,
          page_size: 15,
        });
        const exclude = new Set(excludeIds);
        const items = (res.cases ?? []).map((c) => ({
          id: c.case_id,
  ...
  return (
    <div ref={containerRef} className="relative max-w-md space-y-1">
      <Input
        label={t('workspace.pickers.case')}
        placeholder={t('workspace.pickers.casePlaceholder')}
        value={query}
